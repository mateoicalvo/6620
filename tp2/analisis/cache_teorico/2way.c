...
int main(int argc, const char* argv[]) {
    ...
    char nombre[80];
    sprintf(nombre, "2WAY_%d.csv", DIMENSION);
    ...
    for (int iterB = 0; iterB < 64; ++iterB) {
        for (int iterC = 0; iterC < 64; ++iterC) {
            ...
            for (int i = 0; i < DIMENSION*DIMENSION*(1 + 2*DIMENSION); ++i) {
                ...
                bloques[(2*index) + (rand() % ASOCIATIVIDAD)] = tag;
                encontrado:
                ;
            }
